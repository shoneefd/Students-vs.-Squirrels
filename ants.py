...
class ThrowerAnt(Ant): #fourth year
    ...
    def nearest_bee(self, hive):
        ...
        while(not current_place.bees):
            current_place = current_place.entrance
            count +=1
            if(current_place.entrance  == None):
                return None
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while current_place.exit is not None:
                ...
                if next_place.ant:
                    if next_place.ant.buffed == False:
                        next_place.ant.damage = next_place.ant.damage*2
                        next_place.ant.buffed = True
                    if next_place.ant.container:
                        if next_place.ant.ant and next_place.ant.ant.buffed == False:
                            next_place.ant.ant.damage = next_place.ant.ant.damage*2
                            next_place.ant.ant.buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    def new_bee_action(colony):
        ...
        if duration > 0:
            duration-=1
            return effect(bee_action)(colony)
        if duration == 0:
